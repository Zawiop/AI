...
dirToSym = {'ENSW' : '+', 'ENW' : 'N', 'ENS' : 'F' , 'ESW' : 'T', 'NSW' : 'W', 'EW' : '-', 'EN' : 'V', 'NW' : 'M' , 'ES' : 'S', 'SW' : 'E', 'N': 'U', 'S': 'D', 'E': 'R', 'W': 'L', 'NS' : '|', }

start = f'G{args[0]}' + ('W' + args[1] if args[1].isdigit() else '')
newArgs = [start]
defRwd = 12
width = findW(int(args[0])) if not args[1].isdigit() else args[1]
if args[1].isdigit(): 
    args = args[2:] 
    defRwd = int(args[1])
else: 
...
for val in args:
    if val == 'G0':
        G1 = False
    elif val[0] == 'R' or val[0] == 'r':
        if val[1] == ':':
            defRwd = int(val[2:])
        elif  ':' not in val: 
            newArgs.append(f'V{val[1:]}R{defRwd}')
        else:
            p1, p2 = val.split(':')
            newArgs.append(f'V{p1[1:]}R{p2}')
    elif val[-1] in {'N', 'S', 'E', 'W'}:
        num = (''.join([i for i in val if i.isdigit()]))
        val = val[len(num)+1:]
        for letter in val:
            newArgs.append(f'E~{num}=' + str(int(num) + {'N': -width, 'S': width, 'E': 1, 'W': -1}[letter]))
    else:
        index = int(val[1:])
        newArgs.append(f'V{index}B')
...
def policyCreater(graph):
    ...
    maximum = max(graph[1].values(), key = val)
    for index in graph[1]:
        if graph[1][index] is not None and graph[1][index] < maximum:
            graph[1][index] = None   
...
def main():
    ...
    printBox(policy, graph[2])
    temp = ''
    for jump in jumps:
         
       temp += f'{jump[0]}~{jump[1]};'
    #print(temp[:-1])
    #print(BFS(5, graph))
