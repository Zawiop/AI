...
def corners(brd, tkn):
    t = 1 * (brd[0] == tkn) + 1 * (brd[7] == tkn) + 1 * (brd[56] == tkn) + 1 * (brd[63] == tkn)
    #print (t)
...
def main():
    ...
    if args[0][0] == 'p':
       ...
       for i in range(games):
            ...
            pzl = defPzl
            ol = runGame(pzl, tkn, tkn)
            score = round(((ol.count(tkn)*100)/(ol.count('x') + ol.count('o'))), 2)
            if ol.count(tkn) == 0:
                print(f'Game {i+1} done, you got skunked')
            elif ol.count(oppFinder[tkn]) == 0:
                print(f'Game {i+1} done, you skunked them')
            elif ol.count(tkn) > ol.count(oppFinder[tkn]):
                    avgs.append(score)
                    print(f'Game {i+1} won: {score}%')
                #print(print2D(ol))
            else:
       ...
       for a in range(flukes):
           print(f'Fluke: {round(avgsWithoutFluke.pop(avgsWithoutFluke.index(min(avgsWithoutFluke))), 2)}')
