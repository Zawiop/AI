...
def createDict(puzzle):
        neighbor_vals = {}
        for i in range(len(puzzle)):
                idxToImposs[i] = set()
                for cset_index in constraintDict[i]:
                        for position in contraintSet[cset_index]:
                                if position != i:
                                        idxToImposs[i].add(position)

                neighbor_vals[i] = {puzzle[j] for j in idxToImposs[i] if puzzle[j] != '.'}
...
def updateDict(symbol, idx, dict):
    new_neighbor_vals = {k: dict[k].copy() for k in dict}
    for neighbor in idxToImposs[idx]:
        new_neighbor_vals[neighbor].add(symbol)
...
def bruteForce(pzl, idx, idxToImposs):
    ...
    open_positions = [(len(symbolSet - idxToImposs[i]), i) for i in range(len(pzl)) if pzl[i] == '.']
